...
| **OTA** over Wi-Fi, **HTML dashboard**, easy channel align at boot | **Yes** (or USB alternatives) |
...
Cycle with the Xbox **Mode** button (see [Xbox controls](ESP32/docs/architecture/xbox_ble_controls.md)).
...
Required for **OTA** and **dashboard over Wi-Fi**. **Skip** for router-less ESP-NOW-only demos.
...
Full explanation: [ESP32/docs/networking.md](ESP32/docs/networking.md).
...
1. Power leader + follower.

2. Pair once via ESP-NOW (follower requests, leader accepts).

3. Pair Xbox on leader boot (BLE scan ~4 s).

4. Default profile **ESP-NOW** → **A** to start mirror → **B** to stop.



With a router: same steps; boards can also join LAN for OTA and dashboard.



Optional dashboard (needs network path to leader):
...
| **Router optional, Wi-Fi env vars, OTA** | [ESP32/docs/networking.md](ESP32/docs/networking.md) |
